...
import com.suntrustbank.auth.providers.dtos.EmailRequest;
...
public class NotificationService {
    ...
    public void sendEmail(EmailRequest emailRequest) {
        try {
            rabbitTemplate.convertAndSend(PublisherDetails.EMAIL_ROUTING_KEY.getValue(), new ObjectMapper().writeValueAsString(emailRequest)); // Only routing key required
        } catch (Exception e) {
            log.info("==> Failed to publish message. Error: {}",e.getMessage() , e);
        }
    }
